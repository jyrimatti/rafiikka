...
let reittiUrl                 = (alku, etapit, loppu) => infraAPIUrl + "reitit/kaikki/" + alku + "/" + (etapit && etapit.length > 0 ? etapit.join(',') + '/' : '') + loppu + ".json?propertyName=liikennepaikat,liikennepaikanOsat,seisakkeet,linjavaihteet" + infraAikavali();
let reittiUrlGeojson          = (alku, etapit, loppu) => infraAPIUrl + "reitit/kaikki/" + alku + "/" + (etapit && etapit.length > 0 ? etapit.join(',') + '/' : '') + loppu + ".json?propertyName=geometria" + infraAikavali();
...
let onkoRatakmSijainti  = str => str && str.match(/\(([^)]+)\)\s*(\d+)[+](\d+)/);
let onkoRatanumero      = str => str && str.match(/\(([^)]+)\)/);
let onkoReitti          = str => str && str.match(/^(.*?)\s*(?:=>)\s*(?:(.*)(?:=>))?\s*(.*?)$/);

let onkoJuna  = tunniste => tunniste && tunniste.match(/([0-9]{4}-[0-9]{2}-[0-9]{2})\s*\(?([0-9]*)\)?/);
let onkoOID = tunniste => tunniste && tunniste.match(/^(?:\d+\.)+(\d+)$/);

let onkoInfra = tunniste => tunniste && tunniste.match(/^(?:1\.2\.246\.586\.1\.\d+\.)(.+)$/);
let onkoTREX  = tunniste => tunniste && tunniste.match(/^(?:1\.2\.246\.578\.1\.)(\d+)\.(.+)$/);

let onkoJeti  = tunniste => tunniste && tunniste.match(/^(?:1\.2\.246\.586\.2\.\d+\.|EI|ES|VS|LOI)(.+)$/);
let onkoLOI   = tunniste => tunniste && tunniste.match(/^(?:1\.2\.246\.586\.2\.80\.|LOI)(.+)$/);
let onkoEI    = tunniste => tunniste && tunniste.match(/^(?:1\.2\.246\.586\.2\.81\.|EI)(.+)$/);
let onkoES    = tunniste => tunniste && tunniste.match(/^(?:1\.2\.246\.586\.2\.82\.|ES)(.+)$/);
let onkoVS    = tunniste => tunniste && tunniste.match(/^(?:1\.2\.246\.586\.2\.83\.|VS)(.+)$/);

let onkoRuma  = tunniste => tunniste && tunniste.match(/^(?:1\.2\.246\.586\.7.\d+\.|RT|LR)(.+)$/);
let onkoRT    = tunniste => tunniste && tunniste.match(/^(?:1\.2\.246\.586\.7\.1\.|RT)(.+)$/);
let onkoLR    = tunniste => tunniste && tunniste.match(/^(?:1\.2\.246\.586\.7\.2\.|LR)(.+)$/);
...
let luoInfraAPILinkki = tunniste => `
    <li>
        <a href='${infraAPIUrl}${tunniste}.html'
...
let luoEtj2APILinkki = tunniste => `
    <li>
        <a href='${etj2APIUrl}${tunniste}.html'
