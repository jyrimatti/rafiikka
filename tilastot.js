var kpalueet = [];
getJson(kunnossapitoalueetUrl(), data => {
    kpalueet = Object.values(data).flat().map(d => d.nimi).sort();
});
var ohjausalueet = [];
getJson(liikenteenohjausalueetUrl(), data => {
    ohjausalueet = Object.values(data).flat().map(d => d.nimi).sort();
});
...
let luoTilastoPopupToimialueet            = () => luoTilastoPopup('Toimialueet', toimialueetUrlTilasto(), am4core.color("black").lighten(-0.5), [''], ohjausalueet, ['']);
let luoTilastoPopupTilirataosat           = () => luoTilastoPopup('Tilirataosat', tilirataosatUrlTilasto(), am4core.color("black").lighten(-0.5), [''], kpalueet, ['']);
let luoTilastoPopupLiikennesuunnittelualueet = () => luoTilastoPopup('Liikennesuunnittelualueet', liikennesuunnittelualueetUrlTilasto(), am4core.color("black").lighten(-0.5), [''], [''], ['']);
let luoTilastoPopupPaikantamismerkit      = () => luoTilastoPopup('Paikantamismerkit', paikantamismerkitUrlTilasto(), am4core.color("black").lighten(-0.5), [''], [''], ['']);
...
let luoTilastoPopupEI = () => luoTilastoPopup('Ennakkoilmoitukset', eiUrlTilasto(), am4core.color("orange").lighten(-0.5), ['poistettu', 'luonnos', 'hyväksytty'], ['vek', 'eivek'], asiat, true);
let luoTilastoPopupES = () => luoTilastoPopup('Ennakkosuunnitelmat', esUrlTilasto(), am4core.color("green").lighten(-0.5), ['poistettu', 'peruttu', 'luonnos', 'lähetetty', 'lisätietopyyntö', 'hyväksytty'], estyypit, [''], true);
let luoTilastoPopupVS = () => luoTilastoPopup('Vuosisuunnitelmat', vsUrlTilasto(), am4core.color("violet").lighten(-0.5), ['vuosiohjelmissa (tila poistunut käytöstä)', 'käynnissä (tila poistunut käytöstä)', 'poistettu', 'alustava', 'toteutuu', 'tehty'], [''], ['investointi', 'kunnossapito', 'ulkopuolisen tahon työ'], true);
...
let luoTilastoPopup = (nimi, url, vari, tilat, tyypit, tyonlajit, eiPoistumista) => {
    ...
    on(chart.dataSource.events, "parseended", ev => {
        logDiff("Parsitaan tilastoja", nimi, () => {
            ...
            let data = rows
                .map(x => {
                    let voimassa = limitInterval(x.voimassa || x.objektinVoimassaoloaika).split('/').map(x => new Date(x));
                    ...
                    let ret = {
                        ...
                        tyyppi:        (x.tyyppi || (x.kunnossapitoalue ? x.kunnossapitoalue[0].nimi : undefined) || (x.liikenteenohjausalue ? x.liikenteenohjausalue[0].nimi : undefined) || '').toLowerCase(),
    ...
    let voimassaolon  = luo(nimi, vari, tilat, tyypit, tyonlajit, 'voimassaolo'  , nakyvat, togglet, muut);
    let luontiajan    = luo(nimi, vari, tilat, tyypit, tyonlajit, 'luontiaika'   , nakyvat, togglet, muut);
    let poistumisajan = eiPoistumista ? [] : luo(nimi, vari, tilat, tyypit, tyonlajit, 'poistumisaika', nakyvat, togglet, muut);
...
let luo = (nimi, vari, tilat, tyypit, tyonlajit, ryhmittely, nakyvat, togglet, muut) => {
